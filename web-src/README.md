...
VTS Web 控制台的官方前端源码。功能对等迁移自旧的原生 JS 前端（`web/static/` 旧版），
**不做视觉重设计**：信息架构（四视图 + 设置子 tab）、文案、交互语义与旧版保持一致；
视觉样式沿用既有设计系统 `src/styles/style.css`（`html[data-theme]` 深浅双主题）。
...
│              errors.ts（失败分类）、format.ts、recent.ts（最近来源 localStorage）、
│              theme.ts、desktop.ts（pywebview 桥）、notify.ts
